...
$org_data = $obj->ArrQuery("SELECT * FROM tbl_contract WHERE conid in(".$con_ids.") AND status = 1");
}
...
<?php if (isset($_SESSION['success'])) { ?>
    ...
    <?php }  ?>
<div class="col-md-12">   
<h1>Urgent Updatable Contracts</h1>

</div> 
<div class="col-md-4">
<label for="">Select Organization</label>
    <select name="org_id" id="get_org_contract_urgent" class="form-control mb-2">
...
<table class="table table-bordered ">
    ...
    <?php foreach($org_data as $key=>$value) : ?>
        <tr <?php if(isset($_GET['action']) && $_GET['action'] == 'from_notify' && $_GET['id'] == $value['conid']) {?>style="box-shadow: 0px 0px 20px #2196f3;background:red
"<?php } ?>>
            <td><?=++$key;?></td>
            <td><?=$value['con_title'];?>
...
<script>
    $(document).on('change','#get_org_contract_urgent',function(){
        ...
        $.ajax({
            type : 'post',
            data : orgdata,
            url : 'ajax/ajax.php?action=get_org_contract_in_urgent_bill',
